...
make install                     # Install (default PREFIX=~/.local)
...
- **`lib/persephone/crypt.bash`** — Core encryption logic: `encrypt_name`/`decrypt_name` (filename encryption via AES-256-CBC + URL-safe base64), `encrypt_file`/`decrypt_file` (file content encryption), `encrypt_item`/`decrypt_item` (recursive file/directory processing with dry-run support). These functions use globals (`$VERBOSE`, `$RECURSIVE`, `$PASSWORD`, `$DRY_RUN`) set by `main()`.
- **`lib/persephone/utils.bash`** — General utilities: colored logging (`log_info`, `log_warn`, `log_error`, `log_verbose`, `die`), `command_exists`, `file_readable`, `prompt_password` (masked input with backspace support), `warn_short_password` (warns if <8 chars), `prompt_password_confirm` (double-entry confirmation).
...
- **`tests/test_helper.bash`** — Shared setup: project root resolution, sources `utils.bash`, defines `find_encrypted_file()` and `find_encrypted_dir()` helpers.
- **`tests/test_utils.bats`** — Unit tests for `lib/persephone/utils.bash` functions (`command_exists`, `file_readable`, `warn_short_password`).
